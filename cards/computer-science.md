...
- **In Dijkstra's Dutch national flag problem, why do you increment the index if the current if it's the lowest color, but not if it's the highest color?** Because the lower items have already been ordered, whereas the higher items still need to be ordered, so the loop has to run again at the current index to order the swapped in item.
- **In Dijkstra's Dutch national flag problem, why do you setup the counts for the highest and lowest item at the beginning?** Because the highest item is used in the while loop condition.
