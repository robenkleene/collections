...
- **How do you recursively binary search an array?** Take the start and end as parameters defaulting to the start and end of an array. Set a base case to return nothing if the start is greater than the end. Get the value at the midpoint between the start and the end. If the target is less, recursively call the function with the end as midpoint minus one, if it's greater, recursively call it with the start as the midpoint plus one. Otherwise return the midpoint.
