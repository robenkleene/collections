...
- **How do you traverse a tree iteratively? How do you switch between depth-first and breadth-first?** Create an array with just the root node to use as a queue or stack of nodes to visit, and create a set to track visited nodes. Loop until the stack is empty: Pop the first node, add it to visited, and add its children to the stack. A queue is breadth-first and a stack is depth-first.
